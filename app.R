...
# ---------- Safe SMILES parsing ----------
safe_parse_smiles <- function(smiles) {
  smiles <- gsub("\\s+", "", smiles)  # remove spaces
...
rdkit_descriptors <- function(smiles, prefix) {
  mol <- safe_parse_smiles(smiles)
  if (is.null(mol)) return(NULL)
...
smiles_to_url <- function(smiles) {
  encoded <- URLencode(smiles, reserved = TRUE)  # encode #, +, etc.
  paste0(
...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      ...
      .mol_img {
        border:2px solid #2c2f36; border-radius:12px;
        padding:10px; background:#fff; width:300px; height:300px;
      }
...
server <- function(input, output, session) {
  ...
  prediction <- eventReactive(input$predict_btn, {
    ...
    mol_ph <- safe_parse_smiles(input$phenol)
    mol_py <- safe_parse_smiles(input$pyridine)
    if (is.null(mol_ph) || is.null(mol_py))
      return(list(error=TRUE, msg="Invalid SMILES entered."))
    
    if (!grepl("Cl|Br|I", input$pyridine))
      return(list(error=TRUE, msg="Pyridine must contain Cl, Br, or I."))
    
    phenol_input <- rdkit_descriptors(input$phenol, "Phenol_")
    pyridine_input <- rdkit_descriptors(input$pyridine, "Pyridine_")
  ...
  output$reaction_box <- renderUI({
    ...
    div(class="output_box", style=paste0("background:", color),
        HTML(label))
  })
  ...
  output$prob_box <- renderUI({
    ...
    if (is.null(pred) || pred$error) return(NULL)
    div(id="prob_box",
        paste0("C–C probability: ", round(pred$probability*100,1), "%"))
  })
